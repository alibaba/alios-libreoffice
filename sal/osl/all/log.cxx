    ...
    char const * message, sal_uInt32 backtraceDepth)
{
    ...
    switch (level) {
    case SAL_DETAIL_LOG_LEVEL_INFO:
        YUNOS_LOG(Info, "office", "%s", s.str().c_str());
        break;
    case SAL_DETAIL_LOG_LEVEL_WARN:
        YUNOS_LOG(Warn, "office", "%s", s.str().c_str());
        break;
    case SAL_DETAIL_LOG_LEVEL_DEBUG:
        YUNOS_LOG(Debug, "office", "%s", s.str().c_str());
        break;
    default:
        YUNOS_LOG(Info, "office", "%s", s.str().c_str());
        break;
...
sal_Bool sal_detail_log_report(sal_detail_LogLevel level, char const * area) {
    ...
    if (env == nullptr) {
        env = "+WARN";
#ifdef OFFICEKIT
        env = "+WARN+INFO.desktop.app+INFO.filter.graphic+INFO.sd.docshell";
#endif
    }
